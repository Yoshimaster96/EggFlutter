...
void initAllocTable() {
	for(int i = 0; i < 0x2000; i++) {
		allocOffs[i] = romBuf[0x7F0000+(i<<3)]|(romBuf[0x7F0001+(i<<3)]<<8)|(romBuf[0x7F0002+(i<<3)]<<16)|(romBuf[0x7F0003+(i<<3)]<<24);
		allocSizes[i] = romBuf[0x7F0004+(i<<3)]|(romBuf[0x7F0005+(i<<3)]<<8)|(romBuf[0x7F0006+(i<<3)]<<16)|(romBuf[0x7F0007+(i<<3)]<<24);
	}
}
DWORD romAlloc(DWORD size) {
	//Goal: Find smallest contiguous block of ROM that is at least `size` bytes
	int smallestBufSize = 0x5F0000;
	int smallestBufOffset = 0x200000;
	//Check region immediately at start of extended region
	int smallestAllocOffset0 = 0x7F0000;
	for(int i=0; i<0x2000; i++) {
		if(allocOffs[i] && allocOffs[i]<smallestAllocOffset0) {
			smallestAllocOffset0 = allocOffs[i];
		}
	}
	//TODO: Handle bank boundary
	...
	//TODO: Handle bank boundary
	int thisSize1 = 0x7F0000-smallestAllocOffset1;
...
void writeRomFile(WORD fileNum,BYTE * data,DWORD size) {
	...
	if(offset != 0) {
		...
		romBuf[0x7F0000+(fileNum<<3)] = offset&0xFF;
		romBuf[0x7F0001+(fileNum<<3)] = (offset>>8)&0xFF;
		romBuf[0x7F0002+(fileNum<<3)] = (offset>>16)&0xFF;
		romBuf[0x7F0003+(fileNum<<3)] = (offset>>24)&0xFF;
		romBuf[0x7F0004+(fileNum<<3)] = size&0xFF;
		romBuf[0x7F0005+(fileNum<<3)] = (size>>8)&0xFF;
		romBuf[0x7F0006+(fileNum<<3)] = (size>>16)&0xFF;
		romBuf[0x7F0007+(fileNum<<3)] = (size>>24)&0xFF;
