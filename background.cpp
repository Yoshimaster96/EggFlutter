...
//Needed because main window uses background for HDMA sprites
...
HBITMAP			hbmpBg;
DWORD *			bmpDataBg;
RECT invRect_bg = {0,0,0x400,0x400};
...
void updateEntireScreen_bg() {
	...
	for(int j=0; j<0x40; j++) {
		for(int i=0; i<0x20; i++) {
			...
			dispMap8Tile(bmpDataBg,0x400,0x400,((td>>8)&0xDC)|1,td&0x3FF,{i<<4,j<<4},invRect_bg,false);
		}
	}
	for(int j=0; j<0x20; j++) {
		for(int i=0; i<0x20; i++) {
			...
			dispMap8Tile(bmpDataBg,0x400,0x400,((td>>8)&0xDC)|3,(td&0x3FF)+0x280,{0x200+(i<<4),j<<4},invRect_bg,false);
		}
	}
}
